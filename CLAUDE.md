...
**Default-on caveat**: PLD is **not** flipped on by default at the CLI level — users still pass `--pld`. Bench shows mixed results — strong wins on Gemma-4-E4B heavy-echo (1.50×) but regressions on Qwen3.5-4B heavy-echo (0.97×) and LFM2.5-350M heavy-echo (0.91×). On creative/novel content all models are at parity (≥0.99×). Once `--pld` is set on the server, the **adaptive gate** (see below) decides per-request whether PLD actually runs; explicit `enable_pld: true` in the request body bypasses the gate.
...
- The gate runs in all three request paths: `/v1/chat/completions`, `/v1/messages` (Anthropic), `/v1/responses`.
