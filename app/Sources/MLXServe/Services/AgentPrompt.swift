...
enum AgentPrompt {
    ...
    [
      ...
      {"type":"function","function":{"name":"generate_image","description":"Generate an image from a text prompt and show it INLINE in this chat. It uses the user's SAVED Image generation settings (model, quality, resolution, steps, seed, safe mode) from the Image window — you only supply the prompt, never the model or size. This is a HEAVY one-shot operation (it loads a multi-GB diffusion model and can take a while), so call it only when the user clearly wants an image. The image model must already be downloaded; if it isn't, you'll get a message telling the user to open the Image window once to download it. Example: {\"prompt\": \"a red fox in the snow at golden hour, cinematic\"}","parameters":{"type":"object","properties":{"prompt":{"type":"string","description":"What to draw — a vivid, detailed description of the image."}},"required":["prompt"]}}},
      {"type":"function","function":{"name":"generate_audio","description":"(Coming soon) Generate speech or audio from text. This is NOT available from chat yet — when the user asks, call this so you can tell them it's coming soon and to use the menu-bar tray ▸ Audio generation window for now. Example: {\"text\": \"Hello there\"}","parameters":{"type":"object","properties":{"text":{"type":"string","description":"The text to speak."}},"required":["text"]}}},
      {"type":"function","function":{"name":"generate_video","description":"(Coming soon) Generate a video from a text prompt. This is NOT available from chat yet — when the user asks, call this so you can tell them it's coming soon and to use the menu-bar tray ▸ Video generation window for now. Example: {\"prompt\": \"a timelapse of clouds over a mountain range\"}","parameters":{"type":"object","properties":{"prompt":{"type":"string","description":"What the video should show."}},"required":["prompt"]}}}
    ]
    """#
    ...
    /// image-only this release.
    static let comingSoonAudio = "Audio generation isn't available from chat yet. Use the menu-bar tray ▸ Audio generation window for now."
    static let comingSoonVideo = "Video generation isn't available from chat yet. Use the menu-bar tray ▸ Video generation window for now."
