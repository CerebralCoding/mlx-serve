...
- **Headless sharing for CLI users.** `mlx-serve --serve --lan-share all` shares without the app, and `--lan-discover` shows peer models in `/v1/models` as `<model>@<peer>` and proxies requests to them.
...
- **M5 neural accelerator support.** We now build MLX ourselves with Apple's NAX kernels enabled (the stock package ships them disabled) and added our own NAX kernels for speculative verification, 1.2-2.2x faster than the stock path, drafting up to depth 8 on M5. M1-M4 Macs keep their existing tuned path. Heads up: all builds now require macOS 26.2+.
- **EmbeddingGemma support.** Google's EmbeddingGemma models now work on `/v1/embeddings`, validated at 0.98+ parity vs the reference implementation.
- **Speculative decoding fixes.** Fixed a Gemma BF16 failure with the assistant drafter, a crash loading mxfp8-quantized MTP heads, and cross-request acceptance seeding being stuck off.
